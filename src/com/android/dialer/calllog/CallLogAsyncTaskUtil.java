...
public class CallLogAsyncTaskUtil {
   ...
   /** The enumeration of {@link AsyncTask} objects used in this class. */
     ...
     */
    /*/ freeme.zhaozehong, 13/07/17. for freemeOS
    private static PhoneCallDetails getPhoneCallDetailsForUri(Context context, Uri callUri) {
    /*/
    public static PhoneCallDetails getPhoneCallDetailsForUri(Context context, Uri callUri) {
    //*/
    ...
    //*/ freeme.zhangjunjian, 17-7-17, Delete specified calls from the call log
    public static void deleteCalls(final Context context, final String[] numbers, final int type,
                                   final CallLogAsyncTaskListener callLogAsyncTaskListener) {
        if (sAsyncTaskExecutor == null) {
            initTaskExecutor();
        }
        ...
        sAsyncTaskExecutor.submit(Tasks.DELETE_CALL, new AsyncTask<Void, Void, Void>() {
            @Override
            public Void doInBackground(Void... params) {
                String numberStr = TextUtils.join(",", numbers);
                StringBuffer buffer = new StringBuffer()
                        .append("number in (")
                        .append(numberStr)
                        .append(")");
                if (type != CallLogQueryHandler.CALL_TYPE_ALL) {
                    buffer.append(" and type = ").append(type);
                }
                context.getContentResolver().delete(
                        TelecomUtil.getCallLogUri(context),
    ...
    //*/ freeme.zhangjunjian, 17-7-17, [Union Query] For MTK calllog query, Return the phone call details for a given call log URI
    public static PhoneCallDetails getPhoneCallDetailsForUnionQueryUri(Context context,
                                                                       Uri callUri) {
        ...
        try {
            ...
            return createConferenceCallDetails(context, cursor);
        } finally {
            if (cursor != null) {
                cursor.close();
            }
    ...
    public static PhoneCallDetails createConferenceCallDetails(Context context, Cursor cursor) {
        ...
        //*/ freeme.zhangjunjian, 17-7-17, add extenal data
        details.setDetailsEx(cursor.getInt(CallLogQuery.IS_FREEME_YELLOW_PAGE),
                cursor.getInt(CallLogQuery.SIM_IDX));
        //*/
