...
//*/freeme.zhangjunjian, 20171013. add missed call sim
...
public class MissedCallNotifier {
    ...
    public void updateMissedCallNotification(int count, String number, int slot) {
        final int titleResId;
        final String expandedText;  // The text in the notification's line 1 and 2.
        ...
        if (count == 1) {
            //TODO: look up caller ID that is not in contacts.
            contactInfo = CallLogNotificationsHelper.getInstance(mContext)
                    .getContactInfo(useCallLog ? newestCall.number : number,
                            useCallLog ? newestCall.numberPresentation
                                    : Calls.PRESENTATION_ALLOWED,
                            useCallLog ? newestCall.countryIso : null);
        ...
        int mSimSlot = getSimSlot(slot);
        RemoteViews mMissedCallBigRemoteViews = new RemoteViews(mContext.getPackageName(),
                R.layout.freeme_missed_call_notification);
        if (slot == -1) {
            mMissedCallBigRemoteViews.setViewVisibility(R.id.sim, View.GONE);
        } else {
            mMissedCallBigRemoteViews.setImageViewResource(R.id.sim, mSimSlot);
        }
        mMissedCallBigRemoteViews.setTextViewText(R.id.content, expandedText);
        mMissedCallBigRemoteViews.setTextViewText(R.id.title, mContext.getText(titleResId));
        mMissedCallBigRemoteViews.setImageViewBitmap(R.id.caller_avatar, photoIcon);
        mMissedCallBigRemoteViews.setViewVisibility(R.id.action, View.GONE);
        mMissedCallBigRemoteViews.setTextViewText(R.id.phonelable, mContext.getText(R.string.applicationLabel));

        RemoteViews mMissedCallRemoteViews = new RemoteViews(mContext.getPackageName(),
                R.layout.freeme_missed_call_notification);
        if (slot == -1) {
            mMissedCallRemoteViews.setViewVisibility(R.id.sim, View.GONE);
        } else {
            mMissedCallRemoteViews.setImageViewResource(R.id.sim, mSimSlot);
        }
        mMissedCallRemoteViews.setTextViewText(R.id.content, expandedText);
        mMissedCallRemoteViews.setTextViewText(R.id.title, mContext.getText(titleResId));
        mMissedCallRemoteViews.setImageViewBitmap(R.id.caller_avatar, photoIcon);

        mMissedCallRemoteViews.setTextViewText(R.id.phonelable, mContext.getText(R.string.applicationLabel));
        mFreemeActions.clear();
        if (UserManagerCompat.isUserUnlocked(mContext) && count == 1) {
            if (!TextUtils.isEmpty(number)
                    && !TextUtils.equals(
                    number, mContext.getString(R.string.handle_restricted))) {
                mFreemeActions.add(new FreemeCallAction(
                        mContext.getText(R.string.notification_missedCall_call_back),
                        FreemeCallAction.TEXT_COLOR_ACTION, createCallBackPendingIntent(number)));

                if (!PhoneNumberHelper.isUriNumber(number)) {
                    mFreemeActions.add(new FreemeCallAction(
                            mContext.getText(R.string.notification_missedCall_message),
                            FreemeCallAction.TEXT_COLOR_ACTION, createSendSmsFromNotificationPendingIntent(number)));
                }
            }
        }

        bindAction(mMissedCallRemoteViews);
        Notification.Builder builder = createCustomNotificationBuilder(mContext, timeMs);
        if (Build.VERSION.SDK_INT >= 16) {
            builder.setCustomBigContentView(mMissedCallRemoteViews).setCustomContentView(mMissedCallBigRemoteViews);
        } else {
            builder.setCustomContentView(mMissedCallRemoteViews);
        }
    ...
    public Notification.Builder createCustomNotificationBuilder(Context context, long time) {
    ...
    private int getSimSlot(int slot) {
        return slot == 0 ? R.drawable.freeme_incoming_call_notification_sim1 : R.drawable.freeme_incoming_call_notification_sim2;
    }
    //*/
    ...
    //*/freeme.zhangjunjian, 20171012. add missed call sim
        ...
        if (mFreemeActions.size() > 0) {
            ...
            for (int i = 0; i < mFreemeActions.size(); i++) {
                ...
                RemoteViews child = new RemoteViews(mContext.getPackageName(),
                        R.layout.freeme_missed_call_notification_action);
