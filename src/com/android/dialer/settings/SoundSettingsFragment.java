...
public class SoundSettingsFragment extends PreferenceFragment
    ...
    /*/freeme.zhangjunjian, 20171101. change CheckBoxPreference to SwitchPreference
            ...
            switch (msg.what) {
                case MSG_UPDATE_RINGTONE_SUMMARY:
                    ...
                    if (activity == null || activity.isFinishing()) {
                        return;
                    } else if (DialerUtils.supportDualSimCustomRingtone()
                            && DialerUtils.getSimCount(activity) > 1) {
                        mRingtonePreference.setSummary(
                                getString(R.string.freeme_default_ringtone_summary));
                        break;
                    }
                    //*/
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        //*/ freeme.zhaozehong, 27/07/17. for custom ringtone
        if (onPreferenceTreeClickFreeme(preference)) {
            return true;
        }
        //*/
        if (!SettingsCompat.System.canWrite(getContext())) {
    ...
    private void updateRingtonePreferenceSummary() {
        SettingsUtil.updateRingtoneName(
                ...
                /*/
                DialerUtils.getRingtoneType(getContext()),
                //*/
    ...
    //*/ freeme.zhaozehong, 27/07/17. for custom ringtone
    private boolean onPreferenceTreeClickFreeme(Preference preference){
        if (preference == mRingtonePreference) {
            if (DialerUtils.supportDualSimCustomRingtone()
                    && DialerUtils.getSimCount(getContext()) > 1) {
                Intent intent = new Intent("com.android.settings.action.SIM_SELECT");
                intent.putExtra(Intent.EXTRA_TITLE, getString(R.string.ringtone_title));
                intent.putExtra(RingtoneManager.EXTRA_RINGTONE_TYPE, RingtoneManager.TYPE_RINGTONE);
                getContext().startActivity(intent);
                return true;
            }

            if (DialerUtils.supportCustomRingtone()) {
                Intent intent = new Intent("android.intent.action.RINGTONE");
                intent.putExtra(Intent.EXTRA_TITLE, getString(R.string.ringtone_title));
