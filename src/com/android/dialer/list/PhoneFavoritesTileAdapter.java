...
public class PhoneFavoritesTileAdapter extends BaseAdapter implements
    ...
    protected int getNumStarredContacts(Cursor cursor) {
        cursor.moveToPosition(-1);
        /*/ freeme.zhaozehong, 06/07/17. for non-frequent title
        while (cursor.moveToNext()) {
        ...
        /*/
        nonFrequentItem = false;
        starNamesStrings.clear();
        while (cursor.moveToNext()) {
            String name = cursor.getString(mNamePrimaryIndex);
            if (cursor.getInt(mStarredIndex) == 0) {
                return cursor.getPosition();
            } else {
                if (!starNamesStrings.contains(name)) {
                    starNamesStrings.add(name);
                }
            }
        }
        nonFrequentItem = true;
        //*/
    ...
    public int getCount() {
        ...
        /*/ freeme.zhaozehong, 06/07/17. add non-frequent title
        return mContactEntries.size();
        /*/
        if(nonFrequentItem){
            return mContactEntries.size();
        } else {
            return mContactEntries.size() + 1;
        }
        //*/
    }
    ...
    public ContactEntry getItem(int position) {
        //*/ freeme.zhaozehong, 06/07/17. for show non-frequent title
        if (!nonFrequentItem) {
            int dividerPosition = starNamesStrings.size();
            if (position == dividerPosition) {
                return null;
            } else if (position > dividerPosition) {
                return mContactEntries.get(position - 1);
            } else {
    ...
    public long getItemId(int position) {
        //*/ freeme.zhaozehong, 06/07/17. for non-frequent title
        if (!nonFrequentItem) {
            if (position == starNamesStrings.size()) {
                return -1;
            } else {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        /*/ freeme.zhaozehong, 06/07/17. for freemeOS, UI redesign
        ...
        if (convertView == null) {
            convertView = View.inflate(mContext, R.layout.freeme_phone_favorite_tile_view, null);
            holder = new ViewHolder();
            holder.star = (ImageView) convertView.findViewById(R.id.contact_star_icon);
            holder.name = (TextView) convertView.findViewById(R.id.contact_tile_name);
            holder.photo = (QuickContactBadge) convertView.findViewById(R.id.contact_tile_image);
            holder.frequentTitleDivider = (TextView) convertView.findViewById(R.id.frequentTitleDivider);
            holder.speed_item = (RelativeLayout) convertView.findViewById(R.id.speed_item);

            holder.photo.setOverlay(null);
        ...
        final ContactEntry entry = getItem(position);
        if (entry != null) {
            holder.frequentTitleDivider.setVisibility(View.GONE);
            holder.speed_item.setVisibility(View.VISIBLE);
            ...
            convertView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View arg0) {
                    ...
                    if (TextUtils.isEmpty(entry.phoneNumber)) {
                        // Copy "superclass" implementation
                        mListener.onContactSelected(entry.lookupUri/*getLookupUri()*/,
                                null/*MoreContactUtils.getTargetRectFromView(PhoneFavoriteTileView.this)*/);
                    } else {
                        // When you tap a frequently-called contact, you want to
                        // call them at the number that you usually talk to them
                        // at (i.e. the one displayed in the UI), regardless of
                        // whether that's their default number.
                        mListener.onCallNumberDirectly(entry.phoneNumber);
                    }
                }
            });
            ...
            if (entry == ContactEntry.BLANK_ENTRY) {
                ...
                //holder.star.setVisibility(entry.isFavorite ? View.VISIBLE : View.GONE);
                final boolean isFavorite = entry.isFavorite;
                if (isFavorite) {
                    holder.star.setImageResource(R.drawable.freeme_contacts_star_yes_icon);
                } else {
                    holder.star.setImageResource(R.drawable.freeme_contacts_star_no_icon);
                }
                ...
                holder.star.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        values.put(Contacts.STARRED, !isFavorite);
                        mContext.getContentResolver().update(entry.lookupUri, values, null, null);
                    }
                });
            }

            holder.name.setText(entry.namePrimary);
            Uri mLookupUri = entry.lookupUri;
            holder.photo.assignContactUri(mLookupUri);
            if (mPhotoManager != null) {
                ContactPhotoManager.DefaultImageRequest request = getDefaultImageRequest(
                        entry.namePrimary, entry.lookupKey);
                if (holder.photo != null) {
                    mPhotoManager.loadPhoto(holder.photo, entry.photoUri, -1, false, true, request);
                }
            } else {
                Log.d(TAG, "contactPhotoManager not set");
            }
        } else {
            holder.frequentTitleDivider.setVisibility(View.VISIBLE);
            holder.speed_item.setVisibility(View.GONE);
    ...
    //*/ freeme.zhaozehong, 06/07/17. for freemeOS
    private List<String> starNamesStrings = new ArrayList<String>();
    private boolean nonFrequentItem = false;

    protected ContactPhotoManager.DefaultImageRequest getDefaultImageRequest(String displayName,
                                                                             String lookupKey) {
    ...
    public class ViewHolder {
        ...
        public TextView frequentTitleDivider;
        public RelativeLayout speed_item;
    }
    //*/
}
