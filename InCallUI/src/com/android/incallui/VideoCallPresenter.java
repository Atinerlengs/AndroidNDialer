...
//        InCallPresenter.getInstance().addDetailsListener(this);
    ...
    private void updateVideoCall(Call call) {
        ...
        /// M:for cmcc video ringtone support. when call has CAPABILITY_VIDEO_RINGTONE, we sholud
        /// update show Video Ui. @{
    ...
    private void showVideoUi(Call call, int videoState, int callState) {
        ...
        boolean transparent = callState == Call.State.INCOMING ||
                (callState == Call.State.DIALING
                 /// M: for cmcc video ringtone support. when call has CAPABILITY_VIDEO_RINGTONE,we
                 /// should make display surface visible. @{
                 && call != null
        ...
        if (call != null && VideoUtils.isVideoCall(call)
            ...
            if (VideoProfile.isBidirectional(videoState)) {
                ...
                /// M: for cmcc video ringtone support.FW will notfiy RX video state, when voice
                /// call is able to received video ringtone from network.And CMCC requirements the
                /// local surface need to hide in this case.To avoid the photo blink,so set local
                /// video is visible at first.the local video will hide when has the video ringtone
                /// capalibilty by ui.hidepreview(true).@{
                ...
                ///@}
                 ...
                 /// M: for cmcc video ringtone support.FW will notfiy RX video state, when voice
                 /// call is able to received video ringtone from network.And CMCC requirements the
                 /// local surface need to hide in this case. @{
                if (call != null
   ...
   private void checkForVideoHeldStateChange(Call call) {
    ...
    /**
     * M:for cmcc video ringtone support. when call has CAPABILITY_VIDEO_RINGTONE, we should
     * make display surface visible.And when call has no CAPABILITY_VIDEO_RINGTONE,we should
     * make display surface transparent at dailing state.
     * @param call
     */
    private void checkForVideoRingtoneStateChange(Call call) {
        ...
        /// reset hide preview flag when call has no CAPABILITY_VIDEO_RINGTONE
